...
"""utility rules for defining string-typed flags"""
...
_multi_string_flag = rule(
    implementation = __multi_string_impl,
...
def _string_flag_with_values(
    ...
    Args:
        ...
        enable_all: boolean, True if by all supported values should be set by default.
        visibility: list of Label, defines visiblility of user-facing config settings.
    """
    if multiple:
        _multi_string_flag(
            name = name,
            ...
            build_setting_default = (values + [default] if enable_all else [default]),
...
def _bool_flag(name, default, public = True):
